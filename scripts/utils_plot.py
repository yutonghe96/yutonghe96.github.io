...
def create_travel_map(df_, var='total_days', code_convention='code3', bins=None, colors='Set1', save_path=None):
    ...
    gdf['tooltip_text'] = gdf.apply(
        lambda row: f"{row['country']}: {format_days_to_ymwd(row[var])}" if pd.notnull(row[var]) else f"{row['country']}: No data",
        axis=1
    )
    ...
    <div style="
        ...
        padding: 5px;
    ">
        <i style="background:{set1_colors[0]}; width: 16px; height: 16px; float: left; margin-right: 4px;"></i> days <br>
        <i style="background:{set1_colors[1]}; width: 16px; height: 16px; float: left; margin-right: 4px;"></i> weeks <br>
        <i style="background:{set1_colors[2]}; width: 16px; height: 16px; float: left; margin-right: 4px;"></i> months <br>
        <i style="background:{set1_colors[3]}; width: 16px; height: 16px; float: left; margin-right: 4px;"></i> years
    </div>
    """
